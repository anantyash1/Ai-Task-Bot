...
async def auto_reprioritize():
    ...
    async for task in tasks_collection.find({
        ...
        if hours < 0:
            ...
            new_priority = "Critical"
        elif hours < 8 and PRIORITY_RANK.get(current, 0) < 2:
            new_priority = "High"
        elif hours < 24 and PRIORITY_RANK.get(current, 0) < 1:
            new_priority = "Medium"

        if new_priority != current:
            await tasks_collection.update_one(
                {"_id": task["_id"]},
                {"$set": {"priority": new_priority, "priority_auto_set": True}}
            )
            upgraded += 1

    if upgraded:
        print(f"Auto-priority: upgraded {upgraded} tasks")